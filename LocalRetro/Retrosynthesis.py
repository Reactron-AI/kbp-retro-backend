...
class LocalRetro():
    ...
    def retrosnythesis(self, smiles, top_k = 10, verbose = False):
        predictions = self._predict_raw(smiles, top_k)
        return self._decode_predictions(smiles, predictions, verbose=verbose)

    def retrosnythesis_rank_slice(self, smiles, start_rank = 0, limit = 1, verbose = False):
        end_rank = start_rank + limit
        predictions = self._predict_raw(smiles, end_rank)
        metadata = {
            'root_action_rank': start_rank,
            'root_rank_start': start_rank,
            'root_rank_exhausted': len(predictions) <= start_rank,
            'next_root_rank_start': min(end_rank, len(predictions))
        }
        if metadata['root_rank_exhausted']:
            return self._decode_predictions(smiles, [], verbose=verbose), metadata

        return (
            self._decode_predictions(
                smiles,
                predictions[start_rank:end_rank],
                verbose=verbose
            ),
            metadata
        )

    def _predict_raw(self, smiles, top_k):
        ...
        with torch.no_grad():
            ...
            if top_k is None:
                atom_candidates = atom_logits.numel() - atom_logits.size(0)
                bond_candidates = bond_logits.numel() - bond_logits.size(0)
                top_k = atom_candidates + bond_candidates
    ...
    def _decode_predictions(self, smiles, predictions, verbose = False):
