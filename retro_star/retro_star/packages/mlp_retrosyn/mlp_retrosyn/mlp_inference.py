...
class MLPModel(object):
    def __init__(self,state_path, template_path, device=-1, fp_dim=2048):
        ...
        self.num_templates = len(self.idx2rules)
        self.net.eval()
        self.device = device
        if device >= 0:
            self.net.to(device)

    def _ranked_templates(self, x, topk):
        ...
        if self.device >= 0:
            preds = preds.cpu()
        if topk is None:
            topk = len(self.idx2rules)
        topk = min(topk, len(self.idx2rules))
        probs, idx = torch.topk(preds,k=topk)
        return probs[0], idx[0].numpy().tolist()

    def _run_ranked_templates(self, x, probs, template_ids):
        rule_k = [self.idx2rules[id] for id in template_ids]
        reactants = []
        scores = []
        templates = []
        for i , rule in enumerate(rule_k):
            ...
            try:
                ...
                for reactant in out1:
                    ...
                    scores.append(probs[i].item()/len(out1))
    ...
    def run(self, x, topk=10):
        probs, template_ids = self._ranked_templates(x, topk)
        return self._run_ranked_templates(x, probs, template_ids)

    def run_rank_slice(self, x, start_rank=0, limit=1):
        end_rank = min(start_rank + limit, self.num_templates)
        if start_rank >= end_rank:
            return {
                ...
                'root_rank_exhausted': True,
                'next_root_rank_start': start_rank
            }

        probs, template_ids = self._ranked_templates(x, end_rank)
        result = self._run_ranked_templates(
            x,
            probs[start_rank:end_rank],
            template_ids[start_rank:end_rank]
        )
        if result is None:
            result = {
                'reactants': [],
