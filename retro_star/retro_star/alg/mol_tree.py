...
class MolTree:
    def __init__(self, target_mol, known_mols, value_fn, zero_known_value=True,
        ...
        self.exclude_mols = set(exclude_mols or [])
        self.mol_nodes = []
        self.reaction_nodes = []

        self.root = self._add_mol_node(target_mol, None)
        self.succ = target_mol in known_mols and target_mol not in self.exclude_mols
    ...
    def _add_mol_node(self, mol, parent):
        is_known = mol in self.known_mols and mol not in self.exclude_mols
    ...
    def _add_reaction_and_mol_nodes(self, cost, mols, parent, template, ancestors,
        ...
        reaction_node = ReactionNode(parent, cost, template, template_rank)
        for mol in mols:
    ...
    def expand(self, mol_node, reactant_lists, costs, templates, template_ranks=None):
        ...
        if template_ranks is None:
            template_ranks = [None] * len(costs)
    ...
    def get_best_route(self):
        ...
        while not mol_queue.empty():
            ...
            syn_route.add_reaction(
                ...
                template_rank=best_reaction.template_rank
            )
    ...
    def viz_search_tree(self, viz_file):
        ...
        for output_format in ('pdf', 'png'):
            try:
                ...
                logging.warning('Skipping search tree render: Graphviz "dot" executable not found on PATH.')
                break
